...
(async () => {
  ...
  global.fetch = async () => ({ ok: true, status: 200, text: async () => JSON.stringify([tampered]) });
  const blocked = await invoke("GET", {
    packageId: "health-medical-testing-clinic-aurora",
    contentVersion: "0.1.0"
  });
  ...
  console.log("K7_8_2A_14_API_OPTIONAL_MANIFEST_CHECKSUM=PASS");
