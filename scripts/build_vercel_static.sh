...
for required in solucoes/workforce/index.html assets/css/workforce.css assets/img/predixai-workforce-flow.svg assets/img/social-card.svg;do [[ -f "${DIST_DIR}/${required}" ]]||fail "entrega Workforce ausente: ${required}";done
grep -q 'id="workforce"' "${DIST_DIR}/index.html"||fail "seção Workforce não estática na Home";grep -q 'PredixAI Workforce' "${DIST_DIR}/solucoes/workforce/index.html"||fail "página Workforce inválida";grep -q '/solucoes/workforce/' "${DIST_DIR}/sitemap.xml"||fail "Workforce ausente do sitemap"
