...
assert(projectionSource.includes('.k6-action-bar button'), "FAIL_CLOSED_ACTION_BLOCK_MISSING");
assert(projectionSource.includes("packageProjectionDisabledOriginal"), "FAIL_CLOSED_CONTROL_RESTORE_MISSING");
...
assert(projectionSource.includes("toneRaw"), "TONE_RAW_SYNC_MISSING");
assert(projectionSource.includes("responseLengthRaw"), "RESPONSE_LENGTH_RAW_MISSING");
assert(projectionSource.includes("companyDisplayName"), "COMPANY_PROJECTION_MISSING");
assert(projectionSource.includes('addDefinitionRow(list, "Empresa"'), "REVIEW_COMPANY_ROW_MISSING");
assert(!projectionSource.includes('term === "Segmento"'), "COMPANY_MUST_NOT_REPLACE_SEGMENT");
...
assert(sample.toneRaw === "professional", "TONE_RAW_NOT_PROJECTED");
assert(sample.responseLength === "Respostas curtas", "RESPONSE_LENGTH_NOT_PROJECTED");
assert(sample.responseLengthRaw === "short", "RESPONSE_LENGTH_RAW_NOT_PROJECTED");
...
console.log("FAIL_CLOSED_ACTION_BLOCK=PASS");
...
console.log("CUSTOM_TONE_RAW_SYNC=PASS");
console.log("CUSTOM_RESPONSE_LENGTH_PROJECTED=PASS");
console.log("SEGMENT_SEMANTICS_PRESERVED=PASS");
